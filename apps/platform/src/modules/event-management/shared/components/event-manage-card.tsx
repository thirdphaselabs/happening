...
import { AspectRatio, Flex, Heading, IconButton, Text, Tooltip } from "@radix-ui/themes";
import { formatDate } from "date-fns";
import Image from "next/image";
import Link from "next/link";
import { PlaventiEvent } from "~/trpc/types";

export function EventManageCard({ event }: { event: PlaventiEvent }) {
  return (
        ...
        <Flex className="hover:border-grayA4 rounded-xl border-[1px] border-solid border-white/50 bg-white/50 p-3 pl-4 transition duration-200 ease-in-out hover:shadow-sm">
          <Flex justify="between" className="w-full" gap="4">
            <Flex direction="column" gap="2">
              ...
              <Heading size="5" color="gray" weight="medium" highContrast className="flex items-center gap-2 tracking-tight leading-tight">
